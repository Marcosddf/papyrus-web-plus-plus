...
const outerFlagNodeStyle = (
  ...
  const outerFlagNodeStyle: React.CSSProperties = {
    display: 'flex',
    padding: '0px',
    width: '100%',
    height: '100%',
  ...
  if (selected || hovered) {
    outerFlagNodeStyle.outline = `${theme.palette.selected} solid 1px`;
  }

  return outerFlagNodeStyle;
};
...
export const OuterFlagNode = memo(({ data, id, selected, dragging }: NodeProps<Node<OuterFlagNodeData>>) => {
