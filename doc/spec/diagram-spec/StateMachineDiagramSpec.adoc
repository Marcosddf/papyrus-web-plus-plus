...
If the StateMachine doesn't yet have a Region, we create it and we display it in the StateMachine.
...
If the State doesn't yet have a Region, we create it and we display it in the State.
...
Create a StateMachine inside the Package, owned by the feature **packagedElement**, then it is the same behavior than the creation on a StateMachine.
...
Create a StateMachine inside the BehavioredClassifier, owned by the feature **ownedBehavior**, then it is the same behavior than the creation on a StateMachine.
...
Create a ProtocolStateMachine inside the Interface, owned by the feature **protocol**, then it is the same behavior than the creation on a StateMachine, then we display the ProtocolStateMachine as the root of the diagram (labeled with a &laquo;protocol&raquo; prefix).
