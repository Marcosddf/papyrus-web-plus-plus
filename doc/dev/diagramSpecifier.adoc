...
The builder is hooked into the Papyrus Web framework via the `PapyrusRepresentationDescriptionRegistryConfigurer` class in the `papyrus-web-application` module.
All @service implementing _IPapyrusViewRepresentationDescriptionBuilder_ will be called to build a representation and then registered in _PapyrusRepresentationDescriptionRegistry_.
This registry is then used to install the representation descriptions in each editing context of a Papyrus Web project. 
...
Custom nodes are defined both in the backend (to make them usable in the View DSL) and the frontend in _papyrus-web-components_ (the actual graphical implementation of the node). 
...
===== Frontend
Custom nodes are located in the `papyrus-web-components/src/nodes` directory in the frontend. Each node is defined in a dedicated directory that contains 4 files:
...
[TIP]
The `papyrus-web-components/src/nodes` directory contains various examples of custom nodes used in Papyrus Web.
...
An example of transformation can be found in _papyrus-web-components/src/nodes/NodesDocumentTransform.ts_.
...
On the frontend side, a custom tool is a React component that is added in the palette in project _papyrus-web-components. This component takes a `DiagramPaletteToolContributionComponentProps` as parameter, and sends the GraphQL mutation to the backend. The file `PapyrusPopupToolContribution` shows such component for our _Import Metaclass_ example:
```typescript
